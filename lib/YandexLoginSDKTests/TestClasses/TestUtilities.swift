func SetOfClasses(prefix: String, postfix: String) -> [AnyClass]
{
    ...
    defer { allClasses.deallocate(capacity: numericCast(expectedClassCount)) }
    let actualClassCount = objc_getClassList(AutoreleasingUnsafeMutablePointer<AnyClass?>(allClasses), expectedClassCount)

    return stride(from: allClasses, to: allClasses.advanced(by: numericCast(actualClassCount)), by: 1).flatMap { c in
